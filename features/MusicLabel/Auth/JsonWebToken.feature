@auth
Feature: Managing JsonWebTokens
  In order to request /authentication/jwt endpoint
  As an api-client
  ...
  Scenario: Login OK
    ...
    When I send a "POST" request to "/authentication/jwt" with body:
      """
      {
    ...
    And the header "Location" should match "/^header\+payload:.*\..*$/"
